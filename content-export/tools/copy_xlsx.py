...
HEAD = ["순번", "섹션", "자리", "구분", "묶음", "현재 문구", "변경 문구", "비고", "자리표(건드리지 마세요)"]
W = [6, 26, 16, 8, 6, 62, 62, 22, 46]
...
def build():
    ...
    for key, name in AREAS:
        ...
        for r in data[key]:
            ...
            ws.append([n, label, r["role"], KIND.get(r["role"], "문구"), g,
                       r["text"], None, note, r["xp"] + "|" + r["kind"]])
            row = ws[ws.max_row]
            for c in row:
                c.border = bd
            ...
            row[6].fill = yfill                     # 변경 문구 — 여기에 쓴다
            row[8].font = Font(size=8, color="98A2B0")
    ...
    lines = [
        ...
        ("", ""),
        ("h", "채우는 법"),
        ("p", "노란 칸(변경 문구)에만 쓰시면 됩니다. 그대로 둘 줄은 비워 두세요 — 빈 칸은 '그대로'로 읽습니다."),
