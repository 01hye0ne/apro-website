...
(function(){
  /* 2026-09-16 : 세부 페이지 GNB 가 새 껍데기(.sf-gnb)로 바뀜면서 .gnb-group 을 못 찾아
...
(function(){
  ...
  sets.forEach(function(set){
    ...
    function draw(){
      ...
      /* 현재 쪽을 가운데 두되 양 끝에서는 창을 안쪽으로 민다 */
      var s = Math.max(0, Math.min(cur - (WIN >> 1), last - WIN + 1));
      ...
      var next = btn('›', '다음 페이지');
      next.className = 'arw'; next.disabled = (cur === last);
...
(function(){
  ...
  window.addEventListener('load', alignMega);
